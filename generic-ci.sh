...
case $1 in
  ...
  install-packer)
    case $2 in
      win) PACKER_CURRENT_OS="windows";;
      *) PACKER_CURRENT_OS="linux";;
    esac
    PACKER_CURRENT_VERSION="$(curl -s https://checkpoint-api.hashicorp.com/v1/check/packer | jq -r -M '.current_version')"
    PACKER_URL="https://releases.hashicorp.com/packer/$PACKER_CURRENT_VERSION/packer_${PACKER_CURRENT_VERSION}_${PACKER_CURRENT_OS}_amd64.zip"
    ...
    sha256sum --check --status packer_SHA256SUM_${PACKER_CURRENT_OS}_amd64
    unzip "packer_${PACKER_CURRENT_VERSION}_${PACKER_CURRENT_OS}_amd64.zip"
    ./packer --version
    cd ..
    ;;

  packer-local)
    ./packer/packer build -only=virtualbox-iso local.json
    ;;
